...
void SignVerifyMessageDialog::on_signMessageButton_SM_clicked()
{
    ...
    CBitcoinEuroAddress addr(ui->addressIn_SM->text().toStdString());
    if (!addr.IsValid()) {
...
{
    CBitcoinEuroAddress addr(ui->addressIn_VM->text().toStdString());
    if (!addr.IsValid()) {
    ...
    if (!(CBitcoinEuroAddress(pubkey.GetID()) == addr)) {
        ui->statusLabel_VM->setStyleSheet("QLabel { color: red; }");
