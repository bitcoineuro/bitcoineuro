...
bool CWallet::LoadCScript(const CScript& redeemScript)
{
    ...
    if (redeemScript.size() > MAX_SCRIPT_ELEMENT_SIZE) {
        std::string strAddr = CBitcoinEuroAddress(CScriptID(redeemScript)).ToString();
...
std::map<CTxDestination, std::vector<COutput>> CWallet::ListCoins() const
{
    ...
    // https://github.com/bitcoineuro/bitcoineuro/pull/10340, but that change has been
    // postponed until after https://github.com/bitcoineuro/bitcoineuro/pull/10244 to
...
bool CWallet::CreateTransaction(const std::vector<CRecipient>& vecSend, CWalletTx& wtxNew, CReserveKey& reservekey, CAmount& nFeeRet, int& nChangePosInOut, std::string& strFailReason, const CCoinControl& coin_control, bool sign)
{
    ...
    unsigned int nBytes;
    {
        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            // change transaction isn't always pay-to-bitcoineuro-address
...
bool CWallet::SetAddressBook(const CTxDestination& address, const std::string& strName, const std::string& strPurpose)
{
    ...
    if (!strPurpose.empty() && !CWalletDB(*dbw).WritePurpose(CBitcoinEuroAddress(address).ToString(), strPurpose))
        return false;
    return CWalletDB(*dbw).WriteName(CBitcoinEuroAddress(address).ToString(), strName);
}
...
{
    {
        ...
        std::string strAddress = CBitcoinEuroAddress(address).ToString();
    ...
    CWalletDB(*dbw).ErasePurpose(CBitcoinEuroAddress(address).ToString());
    return CWalletDB(*dbw).EraseName(CBitcoinEuroAddress(address).ToString());
}
 ...
 * https://bitcoineurotalk.org/?topic=54527, or
 * https://github.com/bitcoineuro/bitcoineuro/pull/1393.
 */
...
bool CWallet::AddDestData(const CTxDestination& dest, const std::string& key, const std::string& value)
{
    ...
    return CWalletDB(*dbw).WriteDestData(CBitcoinEuroAddress(dest).ToString(), key, value);
}
...
bool CWallet::EraseDestData(const CTxDestination& dest, const std::string& key)
{
    if (!mapAddressBook[dest].destdata.erase(key))
        return false;
    return CWalletDB(*dbw).EraseDestData(CBitcoinEuroAddress(dest).ToString(), key);
}
