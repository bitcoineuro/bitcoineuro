BitcoinEuro Core version 0.11.0 is now available from:

  <https://bitcoineuro.org/bin/bitcoineuro-core-0.11.0/>
...
defaults to 0.00001. This will cause transactions with fewer BCE/kB fee to be
...
For example, add the following to `bitcoineuro.conf`:
...
related to the blockchain in the bitcoineuro system: the raw blocks as received over 
the network (blk???.dat), the undo data (rev???.dat), the block index and the 
UTXO set (both LevelDB databases). The databases are built from the raw data.

Block pruning allows BitcoinEuro Core to delete the raw block and undo data once 
...
block index and UTXO databases. The minimum was chosen so that BitcoinEuro Core will 
...
For an example script see [bitcoineuro-submittx](https://github.com/laanwj/bitcoineuro-submittx).
...
- #5334 `cf87536` libbitcoineuroconsensus: Add pkg-config support
...
- #6269 `95aca44` gitian: Use the new bitcoineuro-detached-sigs git repo for OSX signatures
...
- #5729 `9c4a5a5` Allow unit changes for read-only BitcoinEuroAmountField
- #5753 `0f44672` Add bitcoineuro logo to about screen
...
- #5809 `46bfbe7` Add bitcoineuro-cli man page
...
- #5849 `41259ca` contrib/init/bitcoineurod.openrc: Compatibility with previous OpenRC init script variables
...
- #6116 `30dc3c1` [OSX] rename BitcoinEuro-Qt.app to BitcoinEuro-Core.app
- #6168 `b3024f0` contrib/linearize: Support linearization of testnet blocks
- #6098 `7708fcd` Update Windows resource files (and add one for bitcoineuro-tx)
