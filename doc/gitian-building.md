...
*Setup instructions for a Gitian build of BitcoinEuro Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the BitcoinEuro
...
- [Building BitcoinEuro Core](#building-bitcoineuro-core)
...
- [LXC](https://linuxcontainers.org/), see also [Gitian host docker container](https://github.com/gdm85/tenku/tree/master/docker/gitian-bitcoineuro-host/README.md).
...
Clone the git repositories for bitcoineuro and Gitian.
...
in the bitcoineuro repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building BitcoinEuro Core
----------------

To build BitcoinEuro Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the bitcoineuro repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/bitcoineuro/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/bitcoineuro/bitcoineuro
...
For example:
```bash
URL=https://github.com/laanwj/bitcoineuro.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit bitcoineuro=${COMMIT} --url bitcoineuro=${URL} ../bitcoineuro/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit bitcoineuro=${COMMIT} --url bitcoineuro=${URL} ../bitcoineuro/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit bitcoineuro=${COMMIT} --url bitcoineuro=${URL} ../bitcoineuro/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the bitcoineuro git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../bitcoineuro/contrib/gitian-descriptors/*|sort|uniq )
...
BCEPATH=/some/root/path/bitcoineuro
SIGPATH=/some/root/path/bitcoineuro-detached-sigs

./bin/gbuild --url bitcoineuro=${BCEPATH},signature=${SIGPATH} ../bitcoineuro/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/bitcoineuro-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/bitcoineuro-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/bitcoineuro-osx-build.assert
```
...
[bitcoineuro-core/gitian.sigs](https://github.com/bitcoineuro-core/gitian.sigs/) repository, or if that's not possible create a pull
