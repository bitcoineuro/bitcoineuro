...
###   bitcoineurocore.org and bitcoineuro.org and compares them.
...
WORKINGDIR="/tmp/bitcoineuro_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://bitcoineurocore.org"
HOST2="https://bitcoineuro.org"
BASEDIR="/bin/"
VERSIONPREFIX="bitcoineuro-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'bitcoineuro-',
...
if [ $? -ne 0 ]; then
   echo "bitcoineuro.org failed to provide signature file, but bitcoineurocore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "bitcoineuro.org and bitcoineurocore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the BitcoinEuro Core binary release signing key installed?"
   fi
