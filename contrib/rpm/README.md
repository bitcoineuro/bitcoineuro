...
The RPM spec file provided here is for BitcoinEuro-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://bitcoineuro.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[BitcoinEuro git repository](https://github.com/bitcoineuro/bitcoineuro) but are not part of
...
Currently only `Source10` is used, for the example `bitcoineuro.conf` file.
...
in the bitcoineuro git repository. At present only an SVG version of the BitcoinEuro
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the bitcoineuro
...
needed when 0.12.1 is released, a proper fix is already in the BitcoinEuro git
master branch.
...
idea to build BitcoinEuro Core against a static version of that library compiled
...
The `bitcoineuro.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `bitcoineuro-server` package. At this point, most RPM based
...
The `bitcoineuro-server` package should have SELinux support. How to properly do
...
BitcoinEuro-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/bitcoineuro/). He has been
packaging BitcoinEuro for Fedora at least since 2012.
...
4. I renamed the `bitcoineuro` package that contains the Qt GUI to `bitcoineuro-core` as
that appears to be how the general population refers to it, in contrast to
`bitcoineuro-xt` or `bitcoineuro-classic`. I wanted to make sure the general population
